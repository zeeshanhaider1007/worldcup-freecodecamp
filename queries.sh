...
echo "$($PSQL "SELECT name FROM games INNER JOIN teams ON games.winner_id = teams.team_id WHERE name LIKE 'Co%' UNION SELECT name FROM games INNER JOIN teams ON games.opponent_id = teams.team_id WHERE name LIKE 'Co%' ORDER BY name")"
